...
def pm(Ai, u, Vπ):
    # make sure the inputs are tensors
    '''Ai = torch.as_tensor(Ai)
    u = torch.as_tensor(u)
    Vπ = torch.as_tensor(Vπ)'''
...
def ddmzm(Ai, u1, u2, Vb1, Vb2, Vπ):
    # make sure the inputs are tensors
    '''Ai = torch.as_tensor(Ai)
    u1 = torch.as_tensor(u1)
    u2 = torch.as_tensor(u2)
    Vb1 = torch.as_tensor(Vb1)
    Vb2 = torch.as_tensor(Vb2)
    Vπ = torch.as_tensor(Vπ)'''
...
def frequencyCombGenerator_MZM_MZM_PM(params, Rs, t, P, Vπ):
    ...
    V3 = torch.as_tensor(V3)
    Phase1 = torch.as_tensor(Phase1)
    Phase2 = torch.as_tensor(Phase2)
    Phase3 = torch.as_tensor(Phase3)
    Vb1 = torch.as_tensor(Vb1)
    Vb2 = torch.as_tensor(Vb2)
    ...
    u1 = V1 * torch.cos(2 * π * Rs * t + Phase1)
    u2 = V2 * torch.cos(2 * π * Rs * t + Phase2)
    u3 = V3 * torch.cos(2 * π * Rs * t + Phase3)
...
def get_psd_ByFFT(signal, Fa, NFFT=16*1024):
    ...
    fft_result = torch.fft.fftshift(torch.fft.fft(signal, n=NFFT))
    power_spectrum = torch.abs(fft_result)**2
    psd = power_spectrum / (NFFT * Fa)
    freqs = torch.fft.fftshift(torch.fft.fftfreq(NFFT, 1 / Fa))

    # Handling zeros in psd (avoiding log(0) issues)
    psd[psd == 0] = torch.finfo(torch.float64).eps # smallest positive number representable by the float64 type
...
def get_indx_peaks(log_Pxx, SpRs, n_peaks):
    ...
    # Find the center index of log_Pxx
    center_indx = log_Pxx.size(0) // 2
    
    # Create an array of offsets for the peaks
    offsets = torch.arange(-(n_peaks // 2), (n_peaks // 2) + 1) * SpRs
    
    # Calculate the indices of the peaks
    indx = center_indx + offsets.to(torch.int)

    # Extract the peaks from log_Pxx
    peaks = log_Pxx[indx]
...
def frequencyCombPeaks_batch(params, args):
    ''' 
    Function to get the peaks of the power spectrum in a batch of frequency comb signals

    Parameters:
    params: list
        Parameters to generate the frequency comb signal in the shape (batch_size, n_parameters)
    args: parameters
        Parameters object with the arguments to generate the frequency comb signal
        Should contain the following attributes: Rs, t, P, Vπ, Fa, NFFT, SpS, n_peaks
    ...
    peaks = torch.zeros((params.shape[0], args.n_peaks))
    for i in range(params.shape[0]):
        peaks[i] = frequencyCombPeaks(params[i], args)

    return peaks

def all_in_a_single_one_function(params, args):
